    ...
    internal class Program
    {
        ...
        private static void LoadEvents()
        {
            try
            {
                ...
                AIBaseClient.OnProcessSpellCast += AboutKSQ;
            }
            catch (Exception ex)
            {
        ...
        private static void AboutKSQ(AIBaseClient sender, AIBaseClientProcessSpellCastEventArgs args)
        {
            try
            {
                if (sender.IsMe)
        ...
        private static void OnDraw(EventArgs args)
        {
            try
            {
                ...
                if (Menu.Item("DrawComboDamage", true).GetValue<bool>())
                    foreach (var e in ObjectManager.Get<AIHeroClient>()
        ...
        private static void CleanLogic()
        {
            try
            {
                if (Menu.Item("JungleStealE", true).GetValue<bool>())
                    if (Menu.Item("OnlyInCleanMode", true).GetValue<bool>() &&
                        Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.LaneClear ||
                        !Menu.Item("OnlyInCleanMode", true).GetValue<bool>())
                    {
                        var mob = MinionManager.GetMinions(E.Range, MinionTypes.All, MinionTeam.Neutral,
                            MinionOrderTypes.MaxHealth);

                        foreach (var m in mob)
                            if (m.CharacterData.Name.Contains("Dragon") || m.CharacterData.Name.Contains("Baron"))
                                if (E.IsKillable(m))
                                    E.Cast();
                    }
            }
            ...
            try
            {
                ...
                if (Menu.Item("HarassE", true).GetValue<bool>())
                    if (Me.ManaPercent > Menu.Item("HarassEMana", true).GetValue<Slider>().Value)
                    {
                        ...
                        if (E.IsReady())
                        {
                            if (Me.Distance(ETarget) > E.Range * 0.8 && ETarget.IsValidTarget(E.Range) &&
                                GetEStackCount(ETarget) >= Menu.Item("HarassEStack", true).GetValue<Slider>().Value)
                                E.Cast();
                            else if (GetEStackCount(ETarget) >= 6) E.Cast();
                        }
                    }
            }
            catch (Exception ex)
                    ...
                    if (Menu.Item("ComboE", true).GetValue<bool>())
                        if (E.IsReady())
                            if (e.IsValidTarget(E.Range))
                                //Chat.Print("Stack E Count -------" + GetEStackCount(e).ToString() + " ------- " + e.CharacterName);

                                if (e.Distance(Me.Position) >= E.Range * 0.7 && e.IsValidTarget(E.Range) &&
                                    GetEStackCount(e) >= Menu.Item("MinComboEStack", true).GetValue<Slider>().Value ||
                                    Menu.Item("UseEIfFullStack", true).GetValue<bool>() && GetEStackCount(e) >= 6)
                                    E.Cast();

                    if (Menu.Item("AutoComboR", true).GetValue<bool>())
                        if (R.IsReady())
                            if (Me.CountEnemiesInRange(Menu.Item("AboutInRRange", true).GetValue<Slider>().Value) >=
                                Menu.Item("AutoRIfEnemiesInRange", true).GetValue<Slider>().Value)
                                R.Cast();
                }
            }
            catch (Exception ex)
        ...
        private static void LoadMenu()
        {
            try
            {
                ...
                Menu.SubMenu("nightmoon.Combo.Menu")
                    .AddItem(new MenuItem("nightmoon.ComboQ.Use", "--------- Q Logic", true))
                    .SetFontStyle(FontStyle.Regular, SColor.Orange);
                Menu.SubMenu("nightmoon.Combo.Menu")
                    .AddItem(new MenuItem("IfEnemyInRangeUseQ", "If Enemy in Range", true).SetValue(true));
                Menu.SubMenu("nightmoon.Combo.Menu")
                    .AddItem(new MenuItem("AboutInQRange", "Search Enemy Range", true).SetValue(
                        new Slider(900, 0, 1800)));
                Menu.SubMenu("nightmoon.Combo.Menu")
                    .AddItem(new MenuItem("nightmoon.ComboW.Use", "--------- W Logic", true))
                    .SetFontStyle(FontStyle.Regular, SColor.SeaGreen);
                Menu.SubMenu("nightmoon.Combo.Menu").AddItem(new MenuItem("ComboW", "Use W", true).SetValue(true));
                Menu.SubMenu("nightmoon.Combo.Menu")
                    .AddItem(new MenuItem("nightmoon.ComboE.Use", "--------- E Logic", true))
                    .SetFontStyle(FontStyle.Regular, SColor.Yellow);
                Menu.SubMenu("nightmoon.Combo.Menu").AddItem(new MenuItem("ComboE", "Use E", true).SetValue(true));
                Menu.SubMenu("nightmoon.Combo.Menu")
                    .AddItem(new MenuItem("UseEIfFullStack", "If Enemy Full Stack", true).SetValue(true));
                Menu.SubMenu("nightmoon.Combo.Menu")
                    .AddItem(
                        new MenuItem("MinComboEStack", "Min E Stack Count(Leave E Range Auto E)", true).SetValue(
                            new Slider(3, 1, 6)));
                Menu.SubMenu("nightmoon.Combo.Menu")
                    .AddItem(new MenuItem("nightmoon.ComboR.Use", "--------- R Logic", true))
                    .SetFontStyle(FontStyle.Regular, SColor.LightCoral);
                Menu.SubMenu("nightmoon.Combo.Menu").AddItem(new MenuItem("AutoComboR", "Use R", true).SetValue(true));
                Menu.SubMenu("nightmoon.Combo.Menu")
                    .AddItem(
                        new MenuItem("AutoRIfEnemiesInRange", "If Enemies in Counts >=", true).SetValue(new Slider(2, 1,
                            5)));
                Menu.SubMenu("nightmoon.Combo.Menu")
                    .AddItem(new MenuItem("AboutInRRange", "Search Enemy Range", true).SetValue(
                        new Slider(800, 0, 1500)));

                Menu.AddSubMenu(new Menu("[FL] Harass Menu", "nightmoon.Harass.Menu"));
                Menu.SubMenu("nightmoon.Harass.Menu")
                    .AddItem(new MenuItem("nightmoon.HarassW.Use", "--------- W Logic", true))
                    .SetFontStyle(FontStyle.Regular, SColor.SeaGreen);
                Menu.SubMenu("nightmoon.Harass.Menu").AddItem(new MenuItem("HarassW", "Use W", true).SetValue(true));
                Menu.SubMenu("nightmoon.Harass.Menu")
                    .AddItem(new MenuItem("HarassWMana", "Harass W Min ManaPer >=", true).SetValue(new Slider(60)));
                Menu.SubMenu("nightmoon.Harass.Menu")
                    .AddItem(new MenuItem("nightmoon.HarassE.Use", "--------- E Logic", true))
                    .SetFontStyle(FontStyle.Regular, SColor.Yellow);
                Menu.SubMenu("nightmoon.Harass.Menu").AddItem(new MenuItem("HarassE", "Use E", true).SetValue(true));
                Menu.SubMenu("nightmoon.Harass.Menu")
                    .AddItem(new MenuItem("HarassEStack", "Min E Stack Count", true).SetValue(new Slider(2, 1, 6)));
                Menu.SubMenu("nightmoon.Harass.Menu")
                    .AddItem(new MenuItem("HarassEMana", "Harass E Min ManaPer >=", true).SetValue(new Slider(60)));

                Menu.AddSubMenu(new Menu("[FL] Misc Menu", "nightmoon.Misc.Menu"));
                Menu.SubMenu("nightmoon.Misc.Menu")
                    .AddItem(new MenuItem("nightmoon.MiscQ.Use", "--------- Q Logic", true))
                    .SetFontStyle(FontStyle.Regular, SColor.Orange);
                Menu.SubMenu("nightmoon.Misc.Menu")
                    .AddItem(new MenuItem("IfKillSomeoneAutoQ", "If Me Kill Enemy & Have others Enemy in Count", true)
                        .SetValue(true));
                Menu.SubMenu("nightmoon.Misc.Menu")
                    .AddItem(new MenuItem("nightmoon.MiscE.Use", "--------- E Logic", true))
                    .SetFontStyle(FontStyle.Regular, SColor.Yellow);
                Menu.SubMenu("nightmoon.Misc.Menu")
                    .AddItem(new MenuItem("JungleStealE", "Use E Steal Dragon/Baron", true).SetValue(true));
                Menu.SubMenu("nightmoon.Misc.Menu")
                    .AddItem(new MenuItem("OnlyInCleanMode", "Only In LaneClear Mode Steal", true).SetValue(true));
                Menu.SubMenu("nightmoon.Misc.Menu")
                    .AddItem(new MenuItem("KillStealE", "Auto E KillSteal", true).SetValue(true));

                Menu.AddSubMenu(new Menu("[FL] Draw Menu", "nightmoon.Draw.Menu"));
                Menu.SubMenu("nightmoon.Draw.Menu")
                    .AddItem(new MenuItem("DrawEnable", "Enable All Drawings", true).SetValue(true));
                Menu.SubMenu("nightmoon.Draw.Menu")
                    .AddItem(new MenuItem("WRange", "W Range", true).SetValue(new Circle(true,
                        System.Drawing.Color.AntiqueWhite)));
                Menu.SubMenu("nightmoon.Draw.Menu")
                    .AddItem(new MenuItem("ERange", "E Range", true).SetValue(new Circle(true,
                        System.Drawing.Color.Coral)));
                Menu.SubMenu("nightmoon.Draw.Menu")
                    .AddItem(new MenuItem("RRange", "R Range", true).SetValue(new Circle(true,
                        System.Drawing.Color.LawnGreen)));
                Menu.SubMenu("nightmoon.Draw.Menu")
                    .AddItem(new MenuItem("DrawComboDamage", "Draw (W+E) Damage", true).SetValue(true));
            }
            catch (Exception ex)
            {

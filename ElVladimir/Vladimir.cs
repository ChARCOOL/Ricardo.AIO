...
namespace ElVladimirReborn
{
    ...
    internal static class Vladimir
    {
        ...
        {
            if (ObjectManager.Player.CharacterData.CharacterName != "Vladimir") return;
        ...
        private static void AreaOfEffectUltimate()
        {
            if (CheckMenu("ElVladimir.Combo.R") && spells[Spells.R].IsReady())
            {
                ...
                if (
                    hits.Any(
                        hit =>
                            hits.Count >= ElVladimirMenu.Menu.Item("ElVladimir.Combo.Count.R").GetValue<Slider>()
                                .Value))
                {
        ...
        private static void OnCombo()
        {
            ...
            if (CheckMenu("ElVladimir.Combo.Ignite") && Player.Distance(target) <= 600
                                                     && Player.GetSummonerSpellDamage(target,
                                                         Damage.DamageSummonerSpell.Ignite) >= target.Health)
        ...
        private static void OnLaneClear()
        {
            ...
            if (spells[Spells.Q].IsReady() && useQ)
            {
                var allMinions = MinionManager.GetMinions(ObjectManager.Player.Position, spells[Spells.Q].Range);
                {
                    foreach (var minion in
                        allMinions.Where(
            ...
            if (spells[Spells.E].IsReady() && Player.Health / Player.MaxHealth * 100 >= playerHp && useE)
            {
                var minions = MinionManager.GetMinions(Player.Position, 800);
                if (minions.Count <= 0) return;

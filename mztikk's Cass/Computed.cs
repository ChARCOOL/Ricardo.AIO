...
namespace mztikksCassiopeia
{
    internal static class Computed
    {
        ...
        public static double ComboDmg(AIBaseClient target)
        {
            var dmg = 0.0;
            if (Spells.Q.IsReady()) dmg += ObjectManager.Player.GetSpellDamage(target, SpellSlot.Q);

            if (Spells.W.IsReady()) dmg += ObjectManager.Player.GetSpellDamage(target, SpellSlot.W);

            if (Spells.E.IsReady()) dmg += Spells.GetEDamage(target);

            if (Spells.R.IsReady()) dmg += ObjectManager.Player.GetSpellDamage(target, SpellSlot.R);

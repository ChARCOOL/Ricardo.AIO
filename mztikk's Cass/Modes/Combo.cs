        ...
        internal static void Execute()
        {
            ...
            if (Config.IsChecked("useRInCombo") && Spells.R.IsReady())
            {
                ...
                if (Config.IsChecked("comboFlashR") && target.IsFacing(ObjectManager.Player) &&
                    ...
                    target.Distance(ObjectManager.Player) <= Spells.R.Range + 400 && Spells.Flash != null &&
                    Spells.Flash.IsReady() && Computed.ComboDmg(target) * Spells.ComboDmgMod > target.Health &&
                ...
                var countFace =
                    HeroManager.Enemies.Count(h =>
                        h.IsValidTarget(Spells.R.Range) && h.IsFacing(ObjectManager.Player) &&
                        Spells.R.WillHit(h, ObjectManager.Player.Position));
                if (countFace >= Config.GetSliderValue("comboMinR") && target.IsValidTarget(Spells.R.Range) &&
                    target.IsFacing(ObjectManager.Player)) Spells.R.Cast(target);
            }

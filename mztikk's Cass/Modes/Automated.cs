    ...
    internal static class Automated
    {
        ...
        public static void EKillSteal()
        {
            if (Spells.E.IsReady())
            {
                ...
                if (entKs != null) Spells.E.Cast(entKs);
            }
        }

        public static void TearStack()
        {
            if (!Spells.Q.IsReady() || ObjectManager.Player.IsRecalling()) return;
            ...
            else
            {
                ...
                else
                {
                    ...
                    Utility.DelayAction.Add(Mainframe.RDelay.Next(30, 70), () => Spells.Q.Cast(rndPlayerPos));
                }
            }
        }
        ...
        {
            if (!Spells.E.IsReady() || ObjectManager.Player.Spellbook.IsAutoAttack) return;
            ...
            if (minionToE != null) Spells.E.Cast(minionToE);
        }
            ...
            if (Config.IsChecked("dontAutoHarassInBush"))
            {
                var bush =
                    ObjectManager.Get<GrassObject>()
                        ...
                        .FirstOrDefault();
                if (bush != null &&
                    ObjectManager.Player.Position.Distance(bush.Position) <= bush.BoundingRadius) return;
            }
        ...
        {
            if (!Spells.Q.IsReady() || ObjectManager.Player.Spellbook.IsAutoAttack) return;
        ...
        {
            if (!Spells.W.IsReady() || ObjectManager.Player.Spellbook.IsAutoAttack) return;

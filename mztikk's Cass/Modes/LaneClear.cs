...
namespace mztikksCassiopeia.Modes
{
    internal static class LaneClear
    {
        ...
        internal static void Execute()
        {
            ...
            if (Config.IsChecked("useQInLC") && Spells.Q.IsReady() && !ObjectManager.Player.Spellbook.IsAutoAttack)
            {
                var qFarmLoc = MinionManager.GetBestCircularFarmLocation(
                    minions.Select(x => x.Position.To2D()).ToList(),
                    Spells.Q.Width,
                    Spells.Q.Range);
                if (qFarmLoc.MinionsHit >= Config.GetSliderValue("minQInLC")) Spells.Q.Cast(qFarmLoc.Position.To3D());
            }

            if (Config.IsChecked("useWInLC") && Spells.W.IsReady() && !ObjectManager.Player.Spellbook.IsAutoAttack)
            {
                        ...
                        .FirstOrDefault(
                            m =>
                                m.IsValidTarget(Spells.E.Range) && Spells.GetEDamage(m) > m.Health
                                                                && (!Config.IsChecked("laneEonP") || m.HasBuffOfType(
                                                                                                      BuffType.Poison)
                                                                                                  || Config.IsChecked(
                                                                                                      "useManaEInLC")
                                                                                                  && ObjectManager
                                                                                                      .Player
                                                                                                      .ManaPercent <=
                                                                                                  Config.GetSliderValue(
                                                                                                      "manaEInLC")));
                if (minToE != null) Spells.E.Cast(minToE);
            }
        }

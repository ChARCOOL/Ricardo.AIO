...
namespace hCamille.Champions
{
    internal class Camille
    {
        ...
        public Camille()
        {
            ...
            Interrupters.OnInterrupter += OnInterrupt;
        }
        ...
        public static bool OnWall =>
            ObjectManager.Player.HasBuff(WallBuff) || Spells.E.Instance.Name == "CamilleEDash2";
        ...
        private void OnInterrupt(ActiveInterrupter interrupter)
        {
            if (interrupter.Sender.IsEnemy && interrupter.DangerLevel > InterrupterDangerLevel.Medium &&
                Utilities.Enabled("e.interrupt"))
            {
                            ...
                            NavMesh.GetCollisionFlags(pos).HasFlag(CollisionFlags.Building))
                        {
                            if (j != 0)
                            {
                                ...
                                do
                                {
                                    ...
                                    pos = new Vector3(
                                        (float)(result.Position.X + middle * cosX),
                                        (float)(result.Position.Y + middle * sinX),
                                        ObjectManager.Player.Position.Z);
                                    if (NavMesh.GetCollisionFlags(pos).HasFlag(CollisionFlags.Wall) ||
                                        NavMesh.GetCollisionFlags(pos).HasFlag(CollisionFlags.Building))
                                        right = middle;
                ...
                var target = interrupter.Sender;
                if (!OnWall)
                    for (var i = 0; i < listPoint.Count - 1; i++)
                        if (listPoint[i].Item1.IsWall() && listPoint[i].Item1.Distance(ObjectManager.Player.Position) <
                                                        Utilities.Slider("wall.distance.to.enemy")
                                                        && listPoint[i].Item1.Distance(target.Position) < 500 &&
                                                        target.IsCastingInterruptableSpell() &&
                                                        !listPoint[i].Item1.To3D().UnderTurret(true))
        ...
        {
            if (gapcloser.Sender != null && gapcloser.Sender.IsEnemy &&
                gapcloser.End.Distance(ObjectManager.Player.Position) < 300
                && Utilities.Enabled("e.anti"))
            {
                ...
                if (!OnWall)
                    for (var i = 0; i < listPoint.Count - 1; i++)
                        if (listPoint[i].Item1.IsWall() && listPoint[i].Item1.Distance(ObjectManager.Player.Position) <
                                                        Utilities.Slider("wall.distance.to.enemy")
                                                        && listPoint[i].Item1.Distance(gapcloser.Sender.Position) >
                                                        400 && !listPoint[i].Item1.To3D().UnderTurret(true))
                        {
                            var i1 = i;
                            var starttick = listPoint[i1].Item2;
                            var startpos = target.Position.To2D();
                            var speed = target.GetSpell(gapcloser.Slot).SData.MissileSpeed;
                            var pathshit = target.Path.OrderBy(x =>
                                    starttick +
                                    (int)(1000 * (new Vector3(x.X, x.Y, x.Z).Distance(startpos.To3D()) / speed)))
                                .FirstOrDefault();
                            ...
                            var endtick = starttick + (int)(1000 * (endpos.Distance(startpos.To3D())
                                                                     / speed));
                            var camilleendtic =
                                starttick + (int)(1000 * (listPoint[i].Item1.Distance(ObjectManager.Player.Position)
                                                           / Spells.E.Speed));
        ...
        private void OnProcess(AIBaseClient sender, AIBaseClientProcessSpellCastEventArgs args)
        {
            if (sender.IsMe)
            {
                if (args.SData.Name == "CamilleW") Spells.W.LastCastAttemptT = Environment.TickCount;

                if (args.SData.Name == "CamilleE" || args.SData.Name == "CamilleEDash2")
                    Spells.E.LastCastAttemptT = Environment.TickCount;

                if (args.SData.Name == "CamilleR") Spells.R.LastCastAttemptT = Environment.TickCount;
            }
        }
        ...
        private void OnDraw(EventArgs args)
        {
            if (Menus.Config.Item("q.draw").GetValue<Circle>().Active && Spells.Q.IsReady())
                Render.Circle.DrawCircle(ObjectManager.Player.Position, Spells.Q.Range,
                    Menus.Config.Item("q.draw").GetValue<Circle>().Color);
            if (Menus.Config.Item("w.draw").GetValue<Circle>().Active && Spells.W.IsReady())
                Render.Circle.DrawCircle(ObjectManager.Player.Position, Spells.W.Range,
                    Menus.Config.Item("w.draw").GetValue<Circle>().Color);
            if (Menus.Config.Item("e.draw").GetValue<Circle>().Active && Spells.E.IsReady())
                Render.Circle.DrawCircle(ObjectManager.Player.Position, Spells.E.Range,
                    Menus.Config.Item("e.draw").GetValue<Circle>().Color);
            if (Menus.Config.Item("r.draw").GetValue<Circle>().Active && Spells.R.IsReady())
                Render.Circle.DrawCircle(ObjectManager.Player.Position, Spells.R.Range,
                    Menus.Config.Item("r.draw").GetValue<Circle>().Color);
        }

        private void OnIssueOrder(AIBaseClient sender, PlayerIssueOrderEventArgs args)
        {
            ...
            {
                var target = TargetSelector.GetTarget(Utilities.Slider("enemy.search.range"),
                    TargetSelector.DamageType.Physical);
                if (target != null)
                {
                    args.Process = false;
                    Player.IssueOrder(GameObjectOrder.MoveTo, target.Position, false);
                }
                else
                {
                    args.Process = true;
        ...
        private void CamilleOnSpellCast(AIBaseClient sender, AIBaseClientProcessSpellCastEventArgs args)
        {
            if (sender.IsMe && args.SData.IsAutoAttack() &&
                Menus.Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo
                && Menus.Config.Item("q.mode").GetValue<StringList>().SelectedIndex == 0 && Spells.Q.IsReady() &&
                Utilities.Enabled("q.combo"))
                Spells.Q.Cast();
        }
                ...
                if (Spells.W.IsReady() && Utilities.Enabled("w.combo") && target.IsValidTarget(Spells.W.Range) &&
                    ...
                    switch (Menus.Config.Item("w.mode").GetValue<StringList>().SelectedIndex)
                    {
                        ...
                        case 1:
                            ...
                            if (predx.Hitchance >= HitChance.Medium) Spells.W.Cast(predx.CastPosition);
                            break;
                    }

                if (Spells.E.IsReady() && Utilities.Enabled("e.combo") &&
                    target.IsValidTarget(Utilities.Slider("enemy.search.range")))
                    if (ObjectManager.Player.CountEnemiesInRange(Utilities.Slider("enemy.search.range")) <=
                        Utilities.Slider("max.enemy.count")
                    ...
                    switch (Menus.Config.Item("r.mode").GetValue<StringList>().SelectedIndex)
                    {
                        case 0:
                            if (target.IsValidTarget(Spells.R.Range) &&
                                target.HealthPercent < Utilities.Slider("enemy.health.percent") &&
                                Utilities.Enabled("r." + target.CharacterName)) Spells.R.CastOnUnit(target);
                            break;

                        case 1:
                            var selectedtarget = TargetSelector.SelectedTarget;
                            if (selectedtarget != null && selectedtarget.IsValidTarget(Spells.R.Range) &&
                                selectedtarget.HealthPercent < Utilities.Slider("enemy.health.percent") &&
                                Utilities.Enabled("r." + selectedtarget.CharacterName))
            ...
            if (!OnWall)
                for (var i = 0; i < listPoint.Count - 1; i++)
                {
                    ...
                    if (listPoint[i].Item1.IsWall() &&
                        ...
                        Utilities.Slider("wall.distance.to.enemy") &&
                        rectangle.IsInside(listPoint[i].Item1)) Spells.E.Cast(listPoint[i].Item1);
                }
        }

        private static void OnMixed()
        {
            if (ObjectManager.Player.ManaPercent < Utilities.Slider("harass.mana")) return;

            var target = TargetSelector.GetTarget(1000, TargetSelector.DamageType.Physical);
            if (target != null)
            {
                if (Spells.Q.IsReady() && Utilities.Enabled("q.combo") &&
                    target.IsValidTarget(ObjectManager.Player.AttackRange)) Spells.Q.Cast();
        ...
        {
            if (ObjectManager.Player.ManaPercent < Utilities.Slider("jungle.mana")) return;

            var mob = MinionManager.GetMinions(ObjectManager.Player.Position,
                Orbwalking.GetRealAutoAttackRange(ObjectManager.Player) + 100, MinionTypes.All, MinionTeam.Neutral,
                MinionOrderTypes.MaxHealth);
            if (mob == null || mob.Count == 0) return;

            if (Spells.Q.IsReady() && Utilities.Enabled("q.jungle") && Spells.Q.IsInRange(mob[0])) Spells.Q.Cast();

            if (Spells.W.IsReady() && Utilities.Enabled("w.jungle")) Spells.W.Cast(mob[0].Position);
        }

        private static void OnClear()
        {
            if (ObjectManager.Player.ManaPercent < Utilities.Slider("clear.mana")) return;

            if (Spells.Q.IsReady() && Utilities.Enabled("q.clear"))
            {
                var minions = MinionManager.GetMinions(ObjectManager.Player.Position, 500).Count;
                if (minions > 4) Spells.Q.Cast();
            }

            if (Spells.Q.IsReady() && Calculation.HasProtocolOneBuff && Utilities.Enabled("q.clear"))
            {
                var minion = MinionManager.GetMinions(ObjectManager.Player.Position, 500).FirstOrDefault();
                if (minion != null && minion.Health < minion.ProtocolDamage()) Spells.Q.Cast();
            }

            if (Spells.Q.IsReady() && Calculation.HasProtocolTwoBuff && Utilities.Enabled("q.clear"))
            {
                var minion = MinionManager.GetMinions(ObjectManager.Player.Position, 500).FirstOrDefault();
                if (minion != null && minion.Health < minion.ProtocolTwoDamage()) Spells.Q.Cast();
            }
            ...
            if (Spells.W.IsReady() && Utilities.Enabled("w.clear"))
            {
                ...
                if (minioncount.MinionsHit >= Utilities.Slider("min.count")) Spells.W.Cast(minioncount.Position);
            }
        }
    }
}
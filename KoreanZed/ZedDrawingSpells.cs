...
namespace KoreanZed
{
    internal class ZedDrawingSpells
    {
        ...
        {
            if (!zedMenu.GetParamBool("koreanzed.drawing.skillranges")) return;
            ...
            {
                range = zedSpells.E.Range +
                        (zedSpells.W.UseOnCombo ? zedSpells.W.IsReady() ? zedSpells.W.Range : 0F : 0F);
            }

            if (range > 0F)
                Render.Circle.DrawCircle(ObjectManager.Player.Position, range, Color.FromArgb(150, Color.DarkGreen));
        }
    }
}
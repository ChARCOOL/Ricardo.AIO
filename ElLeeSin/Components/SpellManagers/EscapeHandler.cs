...
namespace ElLeeSin.Components.SpellManagers
{
    internal class JumpHandler
    {
        ...
        public static AIBaseClient BuffedEnemy =>
            ObjectManager.Get<AIBaseClient>().FirstOrDefault(unit => unit.IsEnemy && unit.HasQBuff());
            ...
            foreach (var pos in JunglePos)
                if (rect != null)
                {
                    if (pos.Distance(ObjectManager.Player.Position) < 2000)
                        Render.Circle.DrawCircle(pos, 100,
        ...
        private static void Escape()
        {
            ...
            if (Misc.IsQOne)
            {
                if (LeeSin.spells[LeeSin.Spells.Q].IsReady())
                    foreach (var pos in JunglePos)
                    {
                        if (rect.IsOutside(pos.To2D())) continue;
                        InitQ = true;

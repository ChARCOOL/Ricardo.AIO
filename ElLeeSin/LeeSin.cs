...
using System.Linq;
using ItemManager = ElLeeSin.Utilities.ItemManager;
...
namespace ElLeeSin
{
    internal class LeeSin
    {
        ...
        public static void CastQ(AIBaseClient target, bool smiteQ = false)
        {
        ...
        public static void CastW(AIBaseClient obj)
        {
            if (500 >= Utils.TickCount - Wardmanager.Wcasttime || Misc.WStage != Wardmanager.WCastStage.First) return;
        ...
        public static void Combo()
        {
            ...
            if (spells[Spells.R].IsReady() && Misc.GetMenuItem("ElLeeSin.Combo.R")
                                           && Misc.GetMenuItem("ElLeeSin.Combo.Q") &&
                                           Misc.GetMenuItem("ElLeeSin.Combo.Q2"))
            {
                ...
                if (qTarget != null)
                    ...
                    {
                        if (spells[Spells.R].CastOnUnit(target)) return;

                        if (Misc.GetMenuItem("ElLeeSin.Combo.StarKill1") && !spells[Spells.R].IsInRange(target)
                                                                         && EnsoulSharp.Common.Geometry.Distance(target,
                                                                             ObjectManager.Player) <
                                                                         600 + spells[Spells.R].Range - 50
                                                                         && ObjectManager.Player.Mana >= 80 &&
            ...
            if (Misc.GetMenuItem("ElLeeSin.Combo.Q2") && !ObjectManager.Player.IsDashing() && target.HasQBuff()
                ...
                if (castQAgain || spells[Spells.Q].GetDamage(target, 1) > target.Health + target.AllShield
                               || Misc.ReturnQBuff()?.Distance<AIBaseClient, AIHeroClient>(target) <
                               EnsoulSharp.Common.Geometry.Distance(ObjectManager.Player, target)
            ...
            if (Misc.GetMenuItem("ElLeeSin.Combo.W"))
            {
                if (Misc.GetMenuItem("ElLeeSin.Combo.Mode.WW")
                    && EnsoulSharp.Common.Geometry.Distance(target, ObjectManager.Player) >
                    Orbwalking.GetRealAutoAttackRange(ObjectManager.Player))
                    Wardmanager.WardJump(target.Position, false, true);

                if (!Misc.GetMenuItem("ElLeeSin.Combo.Mode.WW")
                    && EnsoulSharp.Common.Geometry.Distance(target, ObjectManager.Player) > spells[Spells.Q].Range)
                    Wardmanager.WardJump(target.Position, false, true);
            }

            if (Misc.GetMenuItem("ElLeeSin.Combo.E")) SpellE.CastE();

            if (spells[Spells.W].IsReady() && Misc.GetMenuItem("ElLeeSin.Combo.W2")
                                           && (!Misc.GetMenuItem("ElLeeSin.Combo.W") ||
                                               !Misc.GetMenuItem("ElLeeSin.Combo.Mode.WW")))
            {
                if (Environment.TickCount - lastSpellCastTime <= 500) return;
        ...
        public static void KnockUp()
        {
            ...
            foreach (var enemy in HeroManager.Enemies.Where(e => e.IsValidTarget(1300f)))
            {
                ...
                var endPos = EnsoulSharp.Common.Geometry.Extend(ObjectManager.Player.Position, startPos,
                    EnsoulSharp.Common.Geometry.Distance(ObjectManager.Player, enemy) + LeeSinRKickDistance);

                var rectangle = new Geometry.Polygon.Rectangle(startPos, endPos, LeeSinRKickWidth);
                if (HeroManager.Enemies.Count(x => rectangle.IsInside(x)) >= minREnemies) spells[Spells.R].Cast(enemy);
            }
        }
        ...
        public static void OnGameLoad(EventArgs args)
        {
            try
            {
                if (ObjectManager.Player.CharacterName != "LeeSin") return;
                ...
                AIBaseClient.OnBuffGain += PassiveManager.OnBuffAdd;
                AIBaseClient.OnBuffLose += PassiveManager.OnBuffLose;
        ...
        private static void AfterAttack(AttackableUnit unit, AttackableUnit target)
        {
            ...
            if (!unit.IsMe || targ == null) return;

            if (PassiveStacks > 0) PassiveStacks = PassiveStacks - 1;
            ...
            if (ItemManager.IsActive()) ItemManager.CastItems(targ);
        }

        private static void BeforeAttack(Orbwalking.BeforeAttackEventArgs args)
        {
            var targ = args.Target as AIHeroClient;
            if (!args.Unit.IsMe || targ == null) return;

            if (!spells[Spells.E].IsReady() || !Orbwalker.ActiveMode.Equals(Orbwalking.OrbwalkingMode.Combo)) return;

            if (EnsoulSharp.Common.Geometry.Distance(targ, ObjectManager.Player) <=
                Orbwalking.GetRealAutoAttackRange(ObjectManager.Player))
        ...
        // credits brian
        private static bool CanCastQ2(AIBaseClient target)
        {
        ...
        private static void Game_OnGameUpdate(EventArgs args)
        {
            try
            {
                ...
                if (MyMenu.Menu.Item("starCombo").GetValue<KeyBind>().Active) StarCombo();

                if (MyMenu.Menu.Item("ElLeeSin.Wardjump").GetValue<KeyBind>().Active) Wardmanager.WardjumpToMouse();
                ...
                if (Misc.GetMenuItem("ElLeeSin.Ignite.KS") && igniteSlot != SpellSlot.Unknown)
                {
                    ...
                    if (newTarget != null &&
                        ...
                        ObjectManager.Player.GetSummonerSpellDamage(newTarget, Damage.DamageSummonerSpell.Ignite)
                        > newTarget.Health)
                ...
                if (MyMenu.Menu.Item("InsecEnabled").GetValue<KeyBind>().Active)
                {
                    ...
                    if (newTarget != null) InsecManager.InsecCombo(newTarget);
                }
                else
                {
                ...
                if (MyMenu.Menu.Item("ElLeeSin.Insec.UseInstaFlash").GetValue<KeyBind>().Active)
                {
                    ...
                    if (spells[Spells.R].IsReady() && !target.IsZombie
                                                   && ObjectManager.Player.Spellbook.CanUseSpell(flashSlot) ==
                                                   SpellState.Ready
                                                   && target.IsValidTarget(spells[Spells.R].Range))
                        spells[Spells.R].CastOnUnit(target);
                }
                ...
                if (spells[Spells.R].IsReady() && Misc.GetMenuItem("ElLeeSin.Combo.New")
                                               ...
                                               && !MyMenu.Menu.Item("ElLeeSin.Insec.UseInstaFlash").GetValue<KeyBind>()
                                                   .Active)
                    KnockUp();
        ...
        private static void Harass()
        {
            try
            {
                ...
                if (spells[Spells.Q].IsReady() && Misc.GetMenuItem("ElLeeSin.Harass.Q1"))
                    if (Misc.IsQOne && EnsoulSharp.Common.Geometry.Distance(target, ObjectManager.Player) <
                        spells[Spells.Q].Range)
                        CastQ(target, Misc.GetMenuItem("ElLeeSin.Smite.Q"));

                if (spells[Spells.E].IsReady() && Misc.GetMenuItem("ElLeeSin.Harass.E1"))
                {
                    if (Misc.IsEOne && EnsoulSharp.Common.Geometry.Distance(target, ObjectManager.Player) <
                ...
                if (Misc.GetMenuItem("ElLeeSin.Harass.Wardjump") && EnsoulSharp.Common.Geometry.Distance(
                                                                     ObjectManager.Player, target) < 50
                                                                 && !target.HasQBuff()
                                                                 && (Misc.IsEOne || !spells[Spells.E].IsReady() &&
                                                                     Misc.GetMenuItem("ElLeeSin.Harass.E1"))
                                                                 && (Misc.IsQOne || !spells[Spells.Q].IsReady() &&
                                                                     Misc.GetMenuItem("ElLeeSin.Harass.Q1")))
                {
                    var min =
                        ObjectManager.Get<AIMinionClient>()
                            .Where(a => a.IsAlly && EnsoulSharp.Common.Geometry.Distance(a, ObjectManager.Player) <=
                                        spells[Spells.W].Range)
                            .OrderByDescending(a => EnsoulSharp.Common.Geometry.Distance(a, target))
                            .FirstOrDefault();
        ...
        private static void JungleClear()
        {
            try
            {
                ...
                if (Misc.IsEOne)
                {
                    ...
                    {
                        if (PassiveStacks > 0) return;

                        if (EnsoulSharp.Common.Geometry.Distance(minion, ObjectManager.Player) <
                ...
                {
                    if (PassiveStacks == 0 && EnsoulSharp.Common.Geometry.Distance(minion, ObjectManager.Player) <
                                           spells[Spells.E].Range
                ...
                if (ItemManager.IsActive()) ItemManager.CastItems(minion);

                if (Misc.IsQOne)
                {
                    if (spells[Spells.Q].IsReady() && Misc.GetMenuItem("ElLeeSin.Jungle.Q"))
                    {
                        if (PassiveStacks == 2) return;

                        if (EnsoulSharp.Common.Geometry.Distance(minion, ObjectManager.Player) < spells[Spells.Q].Range)
                        {
                ...
                else
                {
                    if (CanCastQ2(minion) || spells[Spells.Q].GetDamage(minion, 1) > minion.Health
                                          || PassiveStacks == 0
                                          || EnsoulSharp.Common.Geometry.Distance(minion, ObjectManager.Player)
                                          > Orbwalking.GetRealAutoAttackRange(ObjectManager.Player) + 50)
                    {
                        spells[Spells.Q].Cast();
                        lastSpellCastTime = Environment.TickCount;
        ...
        private static void LaneClear()
        {
            try
            {
                ...
                if (spells[Spells.Q].IsReady() && Misc.GetMenuItem("ElLeeSin.Lane.Q"))
                {
                    if (Misc.IsQOne)
                    {
                        if (EnsoulSharp.Common.Geometry.Distance(minions, ObjectManager.Player) <
                            spells[Spells.Q].Range)
                        {
                            ...
                            lastSpellCastTime = Environment.TickCount;
                             ...
                             && (spells[Spells.Q].GetDamage(minions, 1) > minions.Health
                                 || EnsoulSharp.Common.Geometry.Distance(minions, ObjectManager.Player)
                                 > Orbwalking.GetRealAutoAttackRange(ObjectManager.Player) + 50))
                    {
                        spells[Spells.Q].Cast();
                        lastSpellCastTime = Environment.TickCount;
                ...
                if (Misc.GetMenuItem("ElLeeSin.Combo.AAStacks")
                    && PassiveStacks > MyMenu.Menu.Item("ElLeeSin.Combo.PassiveStacks").GetValue<Slider>().Value
                    && Orbwalking.GetRealAutoAttackRange(ObjectManager.Player) >
                    EnsoulSharp.Common.Geometry.Distance(ObjectManager.Player, minions))
                    return;
                ...
                if (spells[Spells.E].IsReady() && Misc.GetMenuItem("ElLeeSin.Lane.E"))
                {
                    ...
                    {
                        if (PassiveStacks == 0 && EnsoulSharp.Common.Geometry.Distance(minions, ObjectManager.Player) <
                                               spells[Spells.E].Range
                ...
                if (ItemManager.IsActive()) ItemManager.CastItems(minions);
            }
            catch (Exception e)
            {
                Console.WriteLine("An error occurred: '{0}'", e);
        ...
        private static void Obj_AI_Base_OnProcessSpellCast(AIBaseClient sender,
            AIBaseClientProcessSpellCastEventArgs args)
        {
            try
            {
                ...
                {
                    case SpellSlot.Q:
                        if (!args.SData.Name.ToLower().Contains("one")) isInQ2 = true;
                        break;

                    case SpellSlot.W:
                        if (args.SData.Name.ToLower().Contains("one")) LastW = Environment.TickCount;
                        break;
                }
                ...
                switch (args.SData.Name.ToLower())
                {
                    ...
                    case "blindmonkqtwo":
                        ...
                        Utility.DelayAction.Add(2900, () => { CheckQ = true; });
                    ...
                    && ObjectManager.Player.Spellbook.CanUseSpell(flashSlot) == SpellState.Ready)
                {
                    ...
                    if (args.SData.Name.Equals("BlindMonkRKick", StringComparison.InvariantCultureIgnoreCase)
                        ...
                        Utility.DelayAction.Add(
                            80,
                            () => ObjectManager.Player.Spellbook.CastSpell(flashSlot,
                                InsecManager.GetInsecPos(target)));
                }
        ...
        private static void OnCreate(GameObject sender, EventArgs args)
        {
                                                         ...
                                                         && sender.Name.ToLower().Contains("ward"))
            {
                ...
                if (EnsoulSharp.Common.Geometry.Distance(ward, Wardmanager.lastWardPos) < 500)
        ...
        {
            if (SmiteBlue.Any(a => Items.HasItem(a))) return "s5_summonersmiteplayerganker";

            if (SmiteRed.Any(a => Items.HasItem(a))) return "s5_summonersmiteduel";
        ...
        private static void StarCombo()
        {
            try
            {
                ...
                if (ItemManager.IsActive()) ItemManager.CastItems(target);

                if (spells[Spells.Q].IsReady())
                {
                    if (Misc.IsQOne)
                    {
                        if (spells[Spells.R].IsReady() && spells[Spells.Q].Cast(target).IsCasted()) return;
                    }
                ...
                if (!spells[Spells.Q].IsReady() || !spells[Spells.R].IsReady() || Misc.IsQOne ||
                    !target.HasQBuff()) return;

                if (spells[Spells.R].IsInRange(target))
                    spells[Spells.R].CastOnUnit(target);
                else if (spells[Spells.W].IsReady())
                    if (EnsoulSharp.Common.Geometry.Distance(target, ObjectManager.Player) > spells[Spells.R].Range
                        && EnsoulSharp.Common.Geometry.Distance(target, ObjectManager.Player) <
                        spells[Spells.R].Range + 580 && target.HasQBuff())

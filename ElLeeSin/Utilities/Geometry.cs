...
namespace ElLeeSin
{
    ...
    public static class Geometry
    {
        ...
        public class Polygon
        {
            ...
            public class Ring : Polygon
            {
                ...
                public void UpdatePolygon(int offset = 0)
                {
                    ...
                    var outRadius = (offset + InnerRadius + OuterRadius)
                                    / (float)Math.Cos(2 * Math.PI / _quality);
            ...
            {
                foreach (var point in polygon.Points) Points.Add(point);
            }
        ...
        public static Vector2[] CircleCircleIntersection(Vector2 center1, Vector2 center2, float radius1, float radius2)
        {
            ...
            if (D > radius1 + radius2 || D <= Math.Abs(radius1 - radius2)) return new Vector2[] { };
        ...
        {
            return unit.Position.To2D().Distance(anotherUnit.Position.To2D(), squared);
        }
        ...
        {
            return unit.Position.To2D().Distance(point.To2D(), squared);
        }
        ...
        {
            return unit.Position.To2D().Distance(point, squared);
        }
        ...
        {
            return v.Distance(to.Position.To2D(), squared);
        }
            ...
            Vector2 lineSegment2End)
        {
            ...
            if (r < 0 || r > 1) return new IntersectionResult();

            var s = (deltaACy * deltaBAx - deltaACx * deltaBAy) / denominator;
            if (s < 0 || s > 1) return new IntersectionResult();
        ...
        {
            var distance = 0f;
            for (var i = 0; i < path.Count - 1; i++) distance += path[i].Distance(path[i + 1]);
            return distance;
        }
        ...
        public static Vector2 PositionAfter(this List<Vector2> self, int t, int s, int delay = 0)
        {
            ...
            for (var i = 0; i <= self.Count - 2; i++)
            {
                ...
                if (d > distance) return from + distance * (to - from).Normalized();
                distance -= d;
            }
        ...
        {
            return new Vector3(v.X, v.Y, ObjectManager.Player.Position.Z);
        }
        ...
        public static Polygon ToPolygon(this List<IntPoint> v)
        {
            ...
            foreach (var point in v) polygon.Add(new Vector2(point.X, point.Y));
            return polygon;
        }
